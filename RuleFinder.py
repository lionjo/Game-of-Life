...
def rulefromDEC(number,noneigh):
    ...
    if(noneigh==5 or noneigh==7 or noneigh==9 or noneigh == 13 or noneigh == 25):
        return np.flip(np.transpose(np.reshape(np.array(list(np.binary_repr(number-1, width = 2*noneigh))),(noneigh,2)))).astype(int)
...
def rulesampler(neighbours):
    if(neighbours==5 or neighbours==7 or neighbours==9 or neighbours == 13 or neighbours == 25):
...
def arrsampler(width,height,sparseness):
    ...
    """
    arr = (np.logical_not(np.random.randint(0,high=sparseness+1, size=(width,height))).astype(bool)).astype(int)
    return arr
